            ...
            @foreach($materials as $itemMaterial)
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        ...
                        <form action="{{ route('materials.destroy', $itemMaterial->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                ...
                <div class="modal fade" id="editMaterialModal{{ $itemMaterial->id }}" tabindex="-1" aria-labelledby="editMaterialModalLabel{{ $itemMaterial->id }}" aria-hidden="true">
                    ...
                    <label for="materialNameInput">Select Material</label>
                    <select class="form-control" id="materialNameInput" name="product_id" required>
<option value="{{ $itemMaterial->product_id  }}" selected>{{ $itemMaterial->product_id  }}</option>
                        @foreach($products as $product)
 <option value="{{ $product->id }}">{{ $product->name }}</option>
                        @endforeach
                    </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="editUnitOfMeasurementInput{{ $itemMaterial->id }}" name="unit_of_measurement" required>
                                            @foreach($units as $unit)
                                                <option value="{{ $unit->abbrev }}" {{ $unit->name == $itemMaterial->unit_of_measurement ? 'selected' : '' }}>
                                                    {{ $unit->abbrev }}
                                                </option>
                                            @endforeach
                                        </select>
    ...
    <a href="{{ route('subelements.items', $item->element_id) }}" class="btn btn-secondary" aria-label="Back" title="Back"><span data-feather="arrow-left-circle"></span></a>
</div>
@endsection
